...
const WorkChart = () => {
  ...
  if (loading && !isLoaded) {
    return (
      <div className="text-sm text-gray-600">Loading request analytics…</div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-sm text-red-600">
        Failed to load request analytics.
      </div>
    );
  }
  ...
  if (requests.length === 0) {
    return (
      <div className="text-sm text-gray-600">No requests found to display.</div>
    );
  }
